...
An abstract class is made by declaring atleast one pure virtual function inside of it
you cannot instantiate this class
meaning you cannot create objects from it
it serves as blueprint
*/
class Replacement
{
...
public:
	 ...
	 */
    /*
        understanding virual functions:
            a virtual function is a function that provides polymorphism
            The decision about which function to call is made at run time
            while non-virtual functions are decided at compile time
            this difference facillitates polymorphism
            it also allows derived classes to provide their own implementations
            and the correct version of the function is determined at run-time
    */
    virtual bool access_page(int page_num, bool is_write = false);

    /**
	 * @brief Accesses a page alreay in physical memory
    ...
    {
        page_table[page_num].valid = true; //valid means that the page is in the pagetable
        page_table[page_num].frame_num = frameCount;//keeps track of what frame the pageEntry is on
        frameCount++;//this keeps track of what frame the pageEntry is on
        pageFaults++;//we faulted so increment our pageFaults counter
    }
	 ...
	 */
    //this is a pure virtual function. This is what makes our class an abstract class
    //by declaring this function as pure, the base class is essentally stating that the derived class must provied its own implementation of this function
